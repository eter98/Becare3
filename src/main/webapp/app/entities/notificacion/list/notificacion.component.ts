...
export class NotificacionComponent implements OnInit {
  ...
  constructor(protected notificacionService: NotificacionService, protected modalService: NgbModal, protected parseLinks: ParseLinks) {
    this.notificacions = [];
  ...
  loadAll(): void {
    ...
    this.notificacionService
      ...
      .subscribe(
        (res: HttpResponse<INotificacion[]>) => {
          this.isLoading = false;
          this.paginateNotificacions(res.body, res.headers);
        },
        () => {
          this.isLoading = false;
        }
  ...
  protected paginateNotificacions(data: INotificacion[] | null, headers: HttpHeaders): void {
    this.links = this.parseLinks.parse(headers.get('link') ?? '');
    if (data) {
      for (const d of data) {
        this.notificacions.push(d);
      }
    }
  }
}
